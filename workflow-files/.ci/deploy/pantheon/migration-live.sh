#!/bin/bash
# Migrate sites in a Pantheon org to new custom upstream
...
# Run with ./migration-live.sh <site-name>
...
# Add manifest plugin to track changes to composer packages
    ...
    cp composer-manifest.yaml ../reports/$SITE/composer-manifest-old-upstream.yaml
    git add . && git commit -m "Add composer manifest and deps to track changes to composer packages."
}
